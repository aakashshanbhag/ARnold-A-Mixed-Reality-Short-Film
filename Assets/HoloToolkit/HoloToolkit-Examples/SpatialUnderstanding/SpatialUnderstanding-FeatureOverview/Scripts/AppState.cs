    ...
    public string PrimaryText
    {
        get
        {
            ...
            if (SpatialUnderstanding.Instance.AllowSpatialUnderstanding)
            {
                ...
                else if (SpatialUnderstanding.Instance.ScanState == SpatialUnderstanding.ScanStates.Finishing)
                {
                    return "Finalizing scan (please wait)";
                }
                else if (SpatialUnderstanding.Instance.ScanState == SpatialUnderstanding.ScanStates.Done && flag == 1)
                {
                    flag = 0;
                    StartCoroutine(WaitTimer());
                    return "Scan complete - Say 'HERE BOY'"; 
                }
                else
                {

...
public class LevelSolver : LineDrawer
{
    ...
    public GameObject ThisIsYourCustomModelInMineItWasATree;
    private GameObject Arnold;
    ...
    public void CreateHologram(Vector3 positionCenter, Quaternion rotation)
    {
        ...
        //if (Physics.Raycast((transform.position), (new Vector3(0, 0, 100)), out hit))
        //{
           // Debug.Log(hit.collider.gameObject);
            //Vector3 tempPos = new Vector3(hit.point.x, (hit.point.y), hit.point.z);
            Arnold = Instantiate(ThisIsYourCustomModelInMineItWasATree, positionCenter, rotation) as GameObject;
           // Arnold.transform.up = GazeManager.Instance.Normal;
            //Main.clip = dogBark;
            //Main.Play();
    ...
    private void MyProcessPlacementResults()
    {
        if (placementResults.Count > 0)
        {
            ...
            CreateHologram(toPlace.Result.Position, rotation);
        }
    }
